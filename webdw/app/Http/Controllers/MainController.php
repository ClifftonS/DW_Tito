...
class MainController extends Controller
{
    public function Home()
    {
        ...
        $CabangCustomer = DB::select('select *, count(*) `Jumlah_Customer` from v_trans group by id_cabang');
        $CabangProduct = DB::select('select *, sum(quantity_product) `Jumlah_Terjual` from v_trans group by id_cabang, product_id, product_name order by id_cabang,`Jumlah_Terjual` desc');
        $CustomerProduct = DB::select('select *, count(customer_name) `Jumlah_Customer` from v_trans group by product_id, product_name');
